...
# global optimum: x(i) = -864.72 ; min F(x) = -792.72*n
# -1000 < x(i) < 1000
# pop - numpy array 1D alebo 2D
# 1D array je interpretovany ako jeden jedinec (1 x D)
def testfn3c(pop: NDArray) -> NDArray:
    pop = np.asarray(pop)

    # !!! Dolezite: 1D povazujeme za 1 jedinca, nie za N bodov 1D funkcie
    if pop.ndim == 1:
        pop = pop.reshape(1, -1)
    ...
    z = pop - x0
    # Fit = sum( -(x-x0)*sin(sqrt(abs(x-x0))) + y0 )
    return np.sum(-(z) * np.sin(np.sqrt(np.abs(z))) + y0, axis=-1)
